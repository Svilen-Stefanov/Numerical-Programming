...
public class Konvergenzordnung {
	 ...
	 * @param h Die Schrittweite h, fuer die die Abschätzung der Ordnung durchgeführt werden soll.
	 * @return Ordnung p
	 */
	public double order(Einschrittverfahren verfahren, double h)
	{
		double eh = error(integrate(verfahren, h));
		double eh2 = error(integrate(verfahren, h/2));
		
		double p = Math.log(eh/eh2)/Math.log(2);
