...
public class Gleitpunktzahl {
	...
	public void normalisiere() {
		...
		int i;
		for (i = 32; i > 0; i--) {
			if( (bitMask & mantisse) == bitMask )
				break;
			bitMask >>>= 1;							//>>>
		}
		
		if( i > sizeMantisse){
			int notRounded = mantisse>>(i-sizeMantisse-1);
			mantisse = notRounded>>1;
			if ((notRounded & 0x1) == 0x1)
				mantisse++;
			exponent += i-sizeMantisse;
		} else if (i < sizeMantisse){
			mantisse <<= sizeMantisse-i;
			exponent -= sizeMantisse-i; 
		} 
		
		if(exponent == -1){				
			exponent = 0;
			mantisse = (1 << sizeMantisse);
		}
		if (exponent > maxExponent)
			setInfinite(vorzeichen);
		if (exponent < 0)	//check if the exponent should be minExponent (0) or real 0
			setNull();
		
		//else it doesn't need to be modified
	}
	...
	public static void denormalisiere(Gleitpunktzahl a, Gleitpunktzahl b) {
		int difference;
		int res = a.compareAbsTo(b);
		if(res >= 1){
			difference = a.exponent - b.exponent;
			a.mantisse <<= difference;
			a.exponent = b.exponent;
		} else if(res < 0) {
			difference = b.exponent - a.exponent;
			b.mantisse <<= difference;
	...
	public Gleitpunktzahl add(Gleitpunktzahl r) {
		Gleitpunktzahl newNumber = new Gleitpunktzahl();
		
		if(this.isNaN() || r.isNaN())
			newNumber.setNaN();
		else if(isInfinite() || r.isInfinite())
			if(isInfinite() && r.isInfinite() && vorzeichen != r.vorzeichen)
				newNumber.setNaN();
			else if (vorzeichen == r.vorzeichen)
				newNumber.setInfinite(vorzeichen);
			else 
				if(isInfinite()) newNumber.setInfinite(vorzeichen); 
				else newNumber.setInfinite(r.vorzeichen);
		else if (isNull() || r.isNull()){
			if(r.isNull())
				newNumber = new Gleitpunktzahl(this);
			else 
				newNumber = new Gleitpunktzahl(r);
			newNumber.normalisiere();
		} else if(Math.abs(exponent - r.exponent) > sizeMantisse) {
			return compareAbsTo(r) > 0 ? new Gleitpunktzahl(this) : new Gleitpunktzahl(r);
		} else if(Math.abs(exponent - r.exponent) == sizeMantisse) { 			// +1
			if(compareAbsTo(r) > 0) {
				newNumber.vorzeichen = vorzeichen;
				newNumber.exponent = exponent;
				newNumber.mantisse = vorzeichen == r.vorzeichen ? mantisse + 1 : mantisse - 1;	
			} else {
				newNumber.vorzeichen = r.vorzeichen;
				newNumber.exponent = r.exponent;
				newNumber.mantisse = vorzeichen == r.vorzeichen ? r.mantisse + 1 : r.mantisse - 1;
			}
		} else if (vorzeichen == r.vorzeichen){
			denormalisiere(this, r);
			long m1 = mantisse, m2 = r.mantisse;
			long res = m1 + m2;
			if(res > Integer.MAX_VALUE || res < Integer.MIN_VALUE){
				res >>>= 1;											//>>>
				newNumber.exponent = exponent + 1;
			}
			else newNumber.exponent = exponent;
				
			newNumber.vorzeichen = vorzeichen;
			newNumber.mantisse = (int)res;
			normalisiere();
			r.normalisiere();
			newNumber.normalisiere();
		} else {
			if(vorzeichen){
				newNumber.mantisse = mantisse;
				newNumber.exponent = exponent;
				newNumber.vorzeichen = !vorzeichen;
				return r.sub(newNumber);
			} else {
				newNumber.mantisse = r.mantisse;
				newNumber.exponent = r.exponent;
				newNumber.vorzeichen = !r.vorzeichen;
	...
	public Gleitpunktzahl sub(Gleitpunktzahl r) {
		...
		if(this.isNaN() || r.isNaN())
			...
			if(isNull() && r.isNull())
				...
				newNumber.exponent = r.exponent;
				newNumber.vorzeichen = !r.vorzeichen;
			} else return new Gleitpunktzahl(this);
		else if(Math.abs(exponent - r.exponent) > sizeMantisse) {
			return compareAbsTo(r) > 0 ? new Gleitpunktzahl(this) : new Gleitpunktzahl(r);
		} else if(Math.abs(exponent - r.exponent) == sizeMantisse) { 			// +1
			if(compareAbsTo(r) > 0) {
			...
			denormalisiere(this, r);
			long m1 = mantisse, m2 = r.mantisse;
			boolean newVorzeichen = m1 > 0;
			long res = m1 - m2;
			newVorzeichen = newVorzeichen ^ (res>0);
			//newVorzeichen = (newVorzeichen && m1 > 0) || (!newVorzeichen && m1 <0);
			if(res > Integer.MAX_VALUE || res < Integer.MIN_VALUE){
				res >>>= 1;											//>>>
				newNumber.exponent = exponent + 1;
			}
			...
			newNumber.vorzeichen = newVorzeichen != vorzeichen;
			newNumber.mantisse = (int)res;
			normalisiere();
			r.normalisiere();
			if(newNumber.mantisse < 0) newNumber.mantisse = -newNumber.mantisse;
			newNumber.normalisiere();
		} else {
			if(vorzeichen){
				newNumber.mantisse = r.mantisse;
				newNumber.exponent = r.exponent;
				newNumber.vorzeichen = !r.vorzeichen;
				return add(newNumber);
			} else {
				newNumber.mantisse = mantisse;
				newNumber.exponent = exponent;
				newNumber.vorzeichen = !vorzeichen;
				return r.add(newNumber);
			}
		}
